...
def extend_route(route, current_connection, railmap):
    ...
    current_connection.visited = True
...
def create_route(route, current_station, current_connection, railmap, connections_per_station):
    ...
    def next_station(
        station1, station2): return station1 if station2 == current_station else station2

    # extend the route with connections untill the tail station of the route has no more connections to choose from
    while True:
        current_station = next_station(
            current_connection.station1, current_connection.station2)
        current_connection = next_connection(
            current_station, route.route, connections_per_station)
...
def create_railmap(connections):
    ...
    single_stations = list(dict(filter(lambda item: len(
        item[1]) == 1, connections_per_station.items())).keys())
    ...
    for i in range(len(single_stations)):
        ...
        if current_connection != None:
            ...
            create_route(route, current_station, current_connection,
                         railmap, connections_per_station)
    ...
    while unsat_stats and route_max(railmap.routes):
        ...
        final_limbs = sum(list(map(lambda station: list(filter(
            lambda connection: connection.visited == False, connections_per_station[station])), unsat_stats)), [])
        ...
        while route_max(railmap.routes) and len(unsat_stats) != 0:
            ...
            create_route(route, current_station,
                         route.route[0], railmap, connections_per_station)
